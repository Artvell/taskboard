"""файл с классом ChangeEntityStatus"""
...
class ChangeEntityStatusView(APIView):
    """меняет параметр статус у указанного Entity"""
    permission_classes = [L2Permission,L3Permission]
    allowed_status = {
        "L2":["3", "4"],
        "L3":["5", "6"]
    }
    def post(self, request, **kwargs): #нужно ли *args
        """метод post, для обработки пост запроса"""
        ...
        if entity_id == "!":
            ...
            try:
                ...
                return Response({"error":"Wrong ID: Entity not found"},
